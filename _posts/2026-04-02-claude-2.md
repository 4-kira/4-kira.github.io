...
Claude 공식 문서([permission-modes](https://code.claude.com/docs/ko/permission-modes))를 읽으면서 생긴 의문들을 중심으로 정리한다.

<br/>

## dontAsk, bypassPermissions 사용
...
permissions 목록에 추가하는 방법은 두 가지다.  
파일을 직접 편집하거나, `default` 모드에서 권한 프롬프트가 떴을 때 `always`를 입력하면 Claude Code가 해당 규칙을 자동으로 `settings.json`에 저장한다.

settings.json 파일이 존재하지 않는 경우 세션에 한정된다.  
저장 우선순위에 대해서는 settings.json 보다 settings.local.json이 우선된다.
...
분류기가 한 세션에서 연속 3회 또는 총 20회 차단하면 `auto` 모드가 중지되고 작업에 대해 다시 프롬프트를 표시한다.

- **CLI**: 상태 영역에 알림이 표시된다. 프롬프트된 작업을 승인하면 카운터가 리셋되어 `auto` 모드를 재개할 수 있다.
...
CLI 플래그는 단발성이다. `settings.json`에 설정하면 별도의 플래그 없이도 항상 해당 모드로 시작한다.
...
`dontAsk`와 `bypassPermissions` 또한 동일하게 설정 가능하다.
...
Claude Code 라이프사이클의 특정 시점에 내가 만든 검사 스크립트를 자동으로 끼워 넣는 장치.  
커스텀 셸 명령, HTTP 엔드포인트, 또는 LLM 프롬프트 등과 관련된 작업.
...
3. hook 스크립트 생성
   `nano .claude/hooks/block-dangerous-bash.sh`
...
7. Claude Code 안에서 확인
   `/hooks`
```

settings 파일 수정 후 Claude Code를 새로 시작하는 게 안전하다.

> hooks의 기본적인 설정을 위한 settings.local.json
...
`command` (셸 명령), `http` (외부 서버에 POST), `prompt` (LLM에게 판단 위임), `agent` (서브에이전트에 위임).
